...
import { gcloudClient, shouldUseGoogleCloud } from '../_lib/services/gcloudClient';
...
const handler = async (req: VercelRequest, res: VercelResponse, data: any) => {
  ...
  try {
    ...
    // Use Google Cloud for all tone analysis
    const shouldUseCloud = shouldUseGoogleCloud(data.text);
    logger.info(`Tone analysis routing: cloud=${shouldUseCloud}, textLength=${data.text.length}`);
    
    let result = await gcloudClient.analyzeTone({
      text: data.text,
      ...
      userProfile: {
        id: userId,
        attachment: attachmentEstimate.primary || undefined,
        secondary: attachmentEstimate.secondary || undefined,
