...
class SuggestionsService {
  ...
  private async buildEnhancedSuggestions(input: {
    ...
    // (1) fold in attachment patterns - use direct attachment style like tone analysis
    // Create attachment estimate directly from client-provided attachment style
    const attachmentEstimate = {
      primary: attachmentStyle as 'secure' | 'anxious' | 'avoidant' | 'disorganized',
      confidence: 1.0, // Full confidence from client-side learning
      scores: {
        secure: attachmentStyle === 'secure' ? 1.0 : 0.0,
        anxious: attachmentStyle === 'anxious' ? 1.0 : 0.0,
        avoidant: attachmentStyle === 'avoidant' ? 1.0 : 0.0,
        disorganized: attachmentStyle === 'disorganized' ? 1.0 : 0.0,
      }
    };
    ...
    logger.debug('suggestions.merged', {
      ...
      conf: attachmentEstimate?.confidence ?? 0
    });
