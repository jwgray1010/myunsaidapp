...
const envSchema = z.object({
  ...
  OPENAI_API_KEY: z.string()
    ...
    .optional() // Temporarily optional for debugging
    .refine(key => !key || key.startsWith('sk-') || process.env.NODE_ENV !== 'production',
...
function validateProductionRequirements(env: Env): void {
  if (env.NODE_ENV === 'production') {
    ...
    // Temporarily disable strict CORS validation for debugging
    // if (env.CORS_ORIGINS === '*') {
    //   issues.push('CORS_ORIGINS cannot be "*" in production');
    // }
    
    // Temporarily disable JWT requirement for debugging  
    // if (!env.JWT_SECRET) {
...
export function getEnv(): Env {
  ...
  try {
    ...
    console.log('  NODE_ENV:', rawEnv.NODE_ENV || 'undefined');
    console.log('  OPENAI_API_KEY exists:', !!rawEnv.OPENAI_API_KEY);
    console.log('  JWT_SECRET exists:', !!rawEnv.JWT_SECRET);
    console.log('  DATABASE_URL exists:', !!rawEnv.DATABASE_URL);
