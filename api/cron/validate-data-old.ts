...
const handler = async (req: VercelRequest, res: VercelResponse) => {
  ...
  try {
    ...
    if (response.ok && healthData.ok) {
      logger.info('Health check passed', {
        duration,
        healthyChecks: healthData.checks?.filter((c: any) => c.ok).length || 0,
        failedChecks: healthData.failing?.length || 0
      });
...
async function testEndpoint(baseUrl: string, path: string, method: string = 'GET', body?: any): Promise<EndpointResult> {
  const startTime = Date.now();
  ...
  try {
    const options: RequestInit = {
      ...
      headers: {
        ...
        'User-Agent': 'Unsaid-HealthCheck/1.0'
      },
